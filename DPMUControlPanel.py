import sys, os, datetime, math, psutil
from os import path
from tkinter.constants import DISABLED
from test.test_typechecks import SubInt
from enum import Enum
from threading import Thread
#from canlib.canlib.constants import canSTAT_SW_OVERRUN, canSTAT_HW_OVERRUN
from _multiprocessing import send
# from can.interfaces.ixxat.constants import FALSE
...
#data Frame(id=1537, data=bytearray(b'/\x00@\x02\x00efg'), dlc=8, flags=<MessageFlag.STD: 2>, timestamp=None)
    ...
    def createDPMULogFile(self, typeOfLog):
        ...
        while (fileCreated == False):
            ...
            if( typeOfLog == OD.I_DEBUG_LOG):
                self.dpmuDebugFileName = dpmuLogPath + "\\DPMU_LOG_" + dateTimeStr + ".hex"
            else:
                serialNumberStr = app.serial_number_entry.get()
                self.dpmuDebugFileName = dpmuLogPath + "\\DPMU_CAN_LOG_" + serialNumberStr + "_" + dateTimeStr + ".hex"
    ...
    def writeDataToLogFile(self, data):
        ...
        if( self.numberOfBlocks%3000 == 0):
            log_box_print("{} ".format( int(self.numberOfBlocks//3000) ) )
    ...
    def closeDPMULogFile(self):
        ...
        log_box_println("Converting log file to Excel xlsx")
        convertDPMULOGFilesToXlsx()
# switch_inrush = canOD()
...
class App(customtkinter.CTk):
    ...
    dpmuCurrentState = 0
    stateReadUpdateTimeMin = 100
    stateReadUpdateTime = 50
    ...
    serialNumberArray = bytearray(30)
    ...
    def __init__(self):
        ...
        rownr += 1
        self.state_pre_initialized_button = customtkinter.CTkButton(self.state_frame, text = "PRE INITIALIZED")
        self.state_pre_initialized_button.grid(row=rownr, column=0, padx=20, pady=10)
        rownr += 1
        self.state_off_button = customtkinter.CTkButton(self.state_frame, command=self.state_off_event, text = "FAULT",  fg_color="red")
        ...
        self.current_max_load_current_entry.insert(0,"1600")
        ...
        self.current_max_ess_current_entry.insert(0,"2.0")
        ...
        self.tabview.add("Serial Number")
        ...
        self.serial_number_frame = customtkinter.CTkFrame(self.tabview.tab("Serial Number"), width=140, corner_radius=10)
        self.serial_number_frame.grid(row=0, column=0, padx=(20, 20), pady=(20, 0), sticky="nsew")
        colnr = 0
        rownr = 0
        
        self.serial_number_frame_label = customtkinter.CTkLabel(master=self.serial_number_frame, text="SERIAL NUMBER (30 BYTES)", font=customtkinter.CTkFont(size=14))
        self.serial_number_frame_label.grid(row=rownr, column=0, columnspan=3, padx=10, pady=10, sticky="")
        rownr += 1
        self.serial_number_entry = customtkinter.CTkEntry(self.serial_number_frame, width=270, justify="left", placeholder_text="0")
        self.serial_number_entry.grid(row=rownr, column=0, padx=(20, 0), pady=(5), sticky="nsew")
        serialStr=""
        for i in range(0,30):
            serialStr = serialStr + chr(i+48) 
        self.serial_number_entry.insert(0,serialStr)
        colnr += 1
        self.serial_number_retrive_button = customtkinter.CTkButton(self.serial_number_frame, command=self.serial_number_retrive_event, width=20, text = "R")
        self.serial_number_retrive_button.grid(row=rownr, column=colnr, padx=(10,10), pady=(10,10), sticky="nsew")
        colnr += 1
        self.serial_number_store_button = customtkinter.CTkButton(self.serial_number_frame, command=self.serial_number_store_event, width=20, text = "W")
        self.serial_number_store_button.grid(row=rownr, column=colnr, padx=(10,10), pady=(10,10), sticky="nsew")


        self.crc_frame = customtkinter.CTkFrame(self.tabview.tab("Serial Number"), width=140, corner_radius=10)
        self.crc_frame.grid(row=1, column=0, padx=(20, 20), pady=(20, 0), sticky="nsew")
        colnr = 0
        rownr = 0
        self.crc_frame_label = customtkinter.CTkLabel(master=self.crc_frame, text="FIRMWARE CRC", font=customtkinter.CTkFont(size=14))
        self.crc_frame_label.grid(row=rownr, column=0, columnspan=3, padx=10, pady=10, sticky="")
        rownr += 1
        colnr = 0
        self.crc_cpu1_label =  customtkinter.CTkLabel(master=self.crc_frame, text="CPU1", font=customtkinter.CTkFont(size=14))
        self.crc_cpu1_label.grid(row=rownr, column=colnr, padx=10, pady=10, sticky="")
        colnr += 1
        self.crc_cpu1_entry = customtkinter.CTkEntry(self.crc_frame, width=80, justify="left", placeholder_text="0")
        self.crc_cpu1_entry.grid(row=rownr, column=colnr, padx=(20, 0), pady=(5), sticky="nsew")
        colnr += 1
        self.crc_cpu2_label =  customtkinter.CTkLabel(master=self.crc_frame, text="CPU2", font=customtkinter.CTkFont(size=14))
        self.crc_cpu2_label.grid(row=rownr, column=colnr, padx=10, pady=10, sticky="")
        colnr += 1
        self.crc_cpu2_entry = customtkinter.CTkEntry(self.crc_frame, width=80, justify="left", placeholder_text="0")
        self.crc_cpu2_entry.grid(row=rownr, column=colnr, padx=(20, 0), pady=(5), sticky="nsew")
        colnr += 1
        self.crc_cpu_read_buton = customtkinter.CTkButton(self.crc_frame, command=self.crc_read_event, width=20, text = "R")
        self.crc_cpu_read_buton.grid(row=rownr, column=colnr, padx=(10,10), pady=(10,10), sticky="nsew")
        ...
        self.canlog_convert_log_excel_button = customtkinter.CTkButton(self.debuglog_frame, command=convertDPMULOGFilesToXlsx, width=20, text = "CONV LOG TO EXCEL")
        self.canlog_convert_log_excel_button.grid(row=rownr, column=0, padx=(0,20), pady=5, sticky="")

        self.canlog_reset_all_flash_button = customtkinter.CTkButton(self.debuglog_frame, command=self.canlog_reset_all_flash_event, width=20, text = "RESET ALL FLASH")
        self.canlog_reset_all_flash_button.grid(row=rownr, column=1, padx=(0,20), pady=5, sticky="")
        ...
        self.serial_number_retrive_event()     
        self.crc_read_event()   
    ...
    def serial_number_store_event(self):
        print("Store serial number")
        serialNumberStr = self.serial_number_entry.get()
        sizeOfSerialNumberStr = len( serialNumberStr )
        if( sizeOfSerialNumberStr  < 30 ):
            print("Serial String has less then 30 chars padding with blanks")
            for i in range(0, (30 - sizeOfSerialNumberStr) ):
                serialNumberStr = serialNumberStr + " "
        save_serial_number_od.setSubIndex(OD.S_SAVE_SERIAL_NUMBER)    
        sendSerialNumberArray = bytearray()
        sendSerialNumberArray.extend( map(ord, serialNumberStr ) )
        #Send 30 Chars
        for i in range(0,10):
            pos=i*3
            print( f"sendSerialNumberArray[%d]=%c", pos, sendSerialNumberArray[pos]) 
            save_serial_number_od.set_data_byte4( sendSerialNumberArray[pos] )
            save_serial_number_od.set_data_byte5(sendSerialNumberArray[pos+1])
            save_serial_number_od.set_data_byte6(sendSerialNumberArray[pos+2])
            save_serial_number_od.set_data_byte7(i)
            save_serial_number_od.sendCanMessage()
            timer.sleep(10/1000)
            
        #Request save to chars to flash
        save_serial_number_od.set_data_byte4(0)
        save_serial_number_od.set_data_byte5(0)
        save_serial_number_od.set_data_byte6(0)        
        save_serial_number_od.set_data_byte7(0xFF)
        save_serial_number_od.sendCanMessage()
    ...
    def serial_number_retrive_event(self):
        print("Retrieve serial number")
        read_serial_number_od.setSubIndex(OD.S_RESTORE_SERIAL_NUMBER )
        for i in range(0, 30):
            self.serialNumberArray[i]=0
        for i in range(0,10):
            read_serial_number_od.set_data_byte4(i)
            read_serial_number_od.set_data_byte5(i)
            read_serial_number_od.set_data_byte6(i) 
            read_serial_number_od.set_data_byte7(i)
            read_serial_number_od.sendCanMessage()      
            timer.sleep(20/1000)
              
    def crc_read_event(self):
        print("Retrieve CRC number CPU1 and CPU2")
        # read_manufacturer_param_od.set_data_byte4(0)
        # read_manufacturer_param_od.set_data_byte5(0)
        # read_manufacturer_param_od.set_data_byte6(0) 
        # read_manufacturer_param_od.set_data_byte7(0)
        read_manufacturer_param_od.setSubIndex(OD.S_RESTORE_MANUFACTURER_DEFINED_DEFAULT_PARAMETERS)
        read_manufacturer_param_od.ccs = 0x40
        read_manufacturer_param_od.paramReadIdx=[False, False]
        read_manufacturer_param_od.paramStr=["",""]
        self.allParamsRead = False
        for j in range(0, 4):
            read_manufacturer_param_od.sendCanMessage()
            timer.sleep(100/1000)
            self.allParamsRead = True
            for i in range(0, len(read_manufacturer_param_od.paramReadIdx) ):
                if( read_manufacturer_param_od.paramReadIdx[i] == False):
                    self.allParamsRead = False
            if (self.allParamsRead == True):
                break
    ...
    def canlog_reset_all_flash_event(self):
        canlog.ccs = 0x2f
        canlog.set_data_byte1( low_byte(OD.I_CAN_LOG))
        canlog.set_data_byte2(high_byte(OD.I_CAN_LOG))
        canlog.subIndex = OD.S_CAN_LOG_RESET
        canlog.set_data_byte4(1)
        canlog.set_data_byte5(0)
        canlog.sendCanMessage()
    ...
    def state_charge_event(self):
        ...
        # state.set_data_byte4(18) # state_balancing testing
        state.sendCanMessage()
    ...
    def state_read_state_event(self):
        ...
        if( canOD.sdoBlockTransferOngoing == 0 ): 
            ...
            match self.dpmuCurrentState:                
                ...
                case [7, 8, 22, 4, 5, 6, 18, 19]:
                    self.energy_cell_charge_read_event()
                    self.bank_charge_read_event()
                    self.bank_remaining_energy_read_event()
                    self.stateReadUpdateTime = 10 * self.stateReadUpdateTimeMin 
                # case [ DPMUState.RegulateInit, DPMUState.Regulate, DPMUState.RegulateVoltageInit, DPMUState.RegulateVoltage ]:
                case [12, 13, 140, 141 ]:
                    self.energy_cell_charge_read_event()
                    self.bank_charge_read_event()
                    self.bank_remaining_energy_read_event()
                    self.stateReadUpdateTime = 4 * self.stateReadUpdateTimeMin
                    
                case [2, 201, 3]: #DPMUState.SoftstartInit, DPMUState.Softstart ]:
                    self.stateReadUpdateTime = 4 * self.stateReadUpdateTimeMin 
                case [15, 16]: #DPMUState.Fault, DPMUState.FaultDelay ]:
                    self.stateReadUpdateTime = 2 * self.stateReadUpdateTimeMin                    
                case _:
                    self.energy_cell_charge_read_event()
                    self.bank_charge_read_event()
                    ...
                    self.stateReadUpdateTime = 20 * self.stateReadUpdateTimeMin
                    
        if( ( canOD.sdoBlock == 0 ) and ( canOD.sdoBlockTransferOngoing == 0) ):
            ...
            try:
                ...
                if msg.flags == canlib.MessageFlag.ERROR_FRAME :
                    ...
                    match msg.id & 0x780 :
                        ...
                        case 0x080:
                            print( "HOUSTON Mensagem de  emergencia recebida: " + str(msg.id & 0x780) )
                            print(msg)
...
def sdo_block_transfer(data):
    ...
    if 1 == canOD.sdoBlockTransferOngoing:
        ...
        if 0 == canOD.ackSeq%4: # block size = 4
            ...
            if  canOD.ongoingCount%64 == 0:
                #log_box_print("{}".format( canOD.numberOfBlocks ) )
    ...
    if 0 != canOD.sdoBlock :
        ...
        if 0x40 == (data[0] & 0xE0):
            ...
            if index == OD.I_DPMU_STATE:                
                ...
                if  app.dpmuCurrentState in [DPMUState.TrickleChargeInit.value, DPMUState.TrickleChargeDelay.value, DPMUState.TrickleCharge.value]:
                # if  app.dpmuCurrentState in [4, 5, 6]:
                    app.chargingFlag = True
                    app.state_charge_button.configure(fg_color="blue")
                elif app.dpmuCurrentState in [DPMUState.ChargeInit.value, DPMUState.Charge.value, DPMUState.ChargeRamp.value, DPMUState.ChargeStop.value]:
                # elif app.dpmuCurrentState in [7, 8, 9, 22]:
                    app.chargingFlag = True
                    app.state_charge_button.configure(fg_color="red")
                #elif app.dpmuCurrentState in [10, 11]:    
                elif app.dpmuCurrentState in [ DPMUState.ChargeConstantVoltageInit.value, DPMUState.ChargeConstantVoltage.value]:
                    app.chargingFlag = True
                    app.state_charge_button.configure(fg_color="yellow")
                elif app.dpmuCurrentState in [DPMUState.BalancingInit.value, DPMUState.Balancing.value]: 
                    app.chargingFlag = True
                    app.state_charge_button.configure(fg_color="white")
                else:
                    app.state_charge_button.configure(fg_color="slategrey")
                
                if  app.dpmuCurrentState in [DPMUState.RegulateVoltageInit.value, DPMUState.RegulateVoltage.value]:
                # if  app.dpmuCurrentState in [140, 141]:    
                    app.state_regulate_button.configure(fg_color="red")
                elif app.dpmuCurrentState in [DPMUState.RegulateInit.value, DPMUState.Regulate.value, DPMUState.RegulateStop.value]:
                ...
                if app.dpmuCurrentState == DPMUState.PreInitialized.value:
                    app.state_pre_initialized_button.configure(fg_color="green")
                else:
                    app.state_pre_initialized_button.configure(fg_color="slategrey")

                

                if (app.dpmuCurrentState == DPMUState.PreInitialized.value):
                    app.state_initialize_button.configure(fg_color="blue")
                elif app.dpmuCurrentState in [DPMUState.Initialize.value, DPMUState.SoftstartInitDefault.value, DPMUState.SoftstartInitRedundant.value, DPMUState.Softstart.value]:
                    app.state_initialize_button.configure(fg_color="white")
                else:
            ...
            if index == OD.I_RESTORE_DEFAULT_PARAMETERS:
                match subIndex:
                    case OD.S_RESTORE_SERIAL_NUMBER:
                        if( data[7] < 10 ):
                            pos = 3*data[7]
                            # snStrPart = ""
                            for i in range(0, 3):
                                app.serialNumberArray[pos+i] = data[i+4]
                                #snStrPart = snStrPart + chr(data[i+4])
                            # print(f"SN[{pos}={snStrPart}")
                            app.serial_number_entry.delete(0,customtkinter.END)
                            serialNumberStr = "".join(map(chr, app.serialNumberArray))
                            app.serial_number_entry.insert(0, serialNumberStr)
                            print("SN:"+serialNumberStr)
                            app.title("DPMU Control Panel - SN: " + serialNumberStr)
                            
                    case OD.S_RESTORE_MANUFACTURER_DEFINED_DEFAULT_PARAMETERS:
                        if( data[7] < len(read_manufacturer_param_od.paramReadIdx) ):
                            read_manufacturer_param_od.paramReadIdx[data[7]] = True
                            read_manufacturer_param_od.paramStr[data[7]] = "0x{0:02X}{1:02X}".format(data[5],data[4])
                        if( data[7] == 0):
                            crc_cpu1 = "0x{0:02X}{1:02X}".format(data[5],data[4])
                            app.crc_cpu1_entry.delete(0, customtkinter.END)
                            app.crc_cpu1_entry.insert(0, crc_cpu1)
                        if( data[7] == 1):
                            crc_cpu2 = "0x{0:02X}{1:02X}".format(data[5],data[4])
                            app.crc_cpu2_entry.delete(0, customtkinter.END)
                            app.crc_cpu2_entry.insert(0, crc_cpu2)
                    case _:
                        pass
            ...
            if index == OD.I_SWITCH_STATE:
                match subIndex:
                    case OD.S_SW_QINRUSH_STATE:
                        # print(f"READ Qinrush STATE:[{msg.data[4]}]", )
                        if 0 == msg.data[4]:
                    ...
                    case OD.S_SW_QLB_STATE:
                        #print(f"READ QLB STATE:[{msg.data[4]}]", )
                        if 0 == msg.data[4]:
                    ...
                    case OD.S_SW_QSB_STATE:
                        #print(f"READ QSB STATE:[{msg.data[4]}]", )
                        if 0 == msg.data[4]:
                    ...
                    case OD.S_SW_QINB_STATE:
                        #print(f"READ QINB STATE:[{msg.data[4]}]", )
                        if 0 == msg.data[4]:
...
def compressFile(unZipedFile, archPath):
    archive_name = f'{archPath}\\{unZipedFile}.zip'
...
def convertDPMULOGFilesToXlsx():
    ...
    DPMULogDir = f"{dpmuLogPath}"
    # DPMULogRreader = r"C:\Users\gferreira\source\repos\DPMULogReader\x64\Release\DPMULogReader.exe"
    #DPMULogRreader = f"{DPMULogDir}\\DPMULogReaderExec\\DPMULogReader.exe"
    DPMULogRreaderExec = f"C:\\Users\\gferreira\\source\\repos\\DPMULogReader\\x64\\Release\\DPMULogReader.exe"
    if ( os.path.isfile(DPMULogRreaderExec) == False ):
        DPMULogRreaderExec = f".\\DPMULogReader.exe"
        if( os.path.isfile(DPMULogRreaderExec) == False ):
            print("Could not find DPMULogReader.exe to convert dpmu hex into csv")
            return
    print("DPMULogReader executable found as:" + DPMULogRreaderExec)
    
    
    DPMUCsvDir = f"{DPMULogDir}\\csv"
    DPMUXlsDir = f"{DPMULogDir}\\xls"
    DPMUArchDir = f"{DPMULogDir}\\arch"
    
    for dir in [DPMUCsvDir, DPMUXlsDir, DPMUArchDir]:
        try:
            os.mkdir(dir)
        except Exception as ex:
            print(f"Could not create {dir}. {ex}") 

    DPMULogHexFileList = glob.glob(f"{dpmuLogPath}\\*.hex")
    ...
    for DPMULogHexFile in DPMULogHexFileList:
        ...
        try:
            print(f"converting hex[{DPMULogHexFile}] to csv")    
            # Custom executable program to convert DPMULogHex in CSV
            subprocess.check_call([DPMULogRreaderExec, DPMULogHexFile])
            ...
            if ( verifyXLSInUse( DPMULogXlsFile ) == True ):
                print(f"Could not create {DPMULogXlsFile}. Excel file  in use" )  
                log_box_println(f"Could not create {DPMULogXlsFile}. Excel file  in use" )  
                return
            
            print(f"Converting csv[{DPMULogCsvFile}] to xlxs:{DPMULogXlsFile}")
            ...
            print(f"Error {ex} converting hex file:{DPMULogHexFile}")
            return
        ...
        try:
            print(f"Compressing file {DPMULogHexFile} into {DPMUArchDir}\\{DPMULogHexFile}.zip")
            compressFile(DPMULogHexFile, DPMUArchDir)
...
if __name__ == "__main__":
    ...
    read_serial_number_od              = canOD(id=nodeId, ccs=0x40, indexHigh=high_byte(OD.I_RESTORE_DEFAULT_PARAMETERS), indexLow=low_byte(OD.I_RESTORE_DEFAULT_PARAMETERS), subIndex=OD.S_RESTORE_SERIAL_NUMBER)
    save_serial_number_od              = canOD(id=nodeId, ccs=0x23, indexHigh=high_byte(OD.I_STORE_PARAMETERS), indexLow=low_byte(OD.I_STORE_PARAMETERS), subIndex=OD.S_SERIAL_NUMBER)

    read_manufacturer_param_od         = canOD(id=nodeId, ccs=0x40, indexHigh=high_byte(OD.I_RESTORE_DEFAULT_PARAMETERS), indexLow=low_byte(OD.I_RESTORE_DEFAULT_PARAMETERS), subIndex=OD.S_RESTORE_MANUFACTURER_DEFINED_DEFAULT_PARAMETERS)
